...
describe("Election Contract", () => {
    ...
    it("Voter Registration", async () => {
        ...
        // register voter and retrieve generated hash via callStatic
        const hash = await hardhatElection.callStatic.registerVoter(_electionID, _voterName, _voterNid, _currentTimeStamp);
    ...
    it("Give Vote", async () => {
        ...
        // register voter and get hash (callStatic returns the value without sending)
        const hash = await hardhatElection.callStatic.registerVoter(_electionID, _voterName, _voterNid, _currentTimeStamp);
        await hardhatElection.registerVoter(_electionID, _voterName, _voterNid, _currentTimeStamp);

        let allElections = await hardhatElection.getElections();
        // give vote using a timestamp inside the election window (just after start)
        const voteTimestamp = _startTime + 1;
        await hardhatElection.giveVote(_electionID, hash, allElections[0].candidates[0].hash, voteTimestamp);
